...
`backend/.env` (gitignored, `.env.example` has the shape) needs `DATABASE_URL`, `JWT_SECRET`, `PORT`, `FRONTEND_ORIGIN`, `NODE_ENV`. `DATABASE_URL` is now a hard requirement enforced at startup by `config/env.js`: since Prisma 7 the schema no longer carries the connection string, so nothing else can supply it. `NODE_ENV` is not optional in practice even though `config/env.js` defaults it to `development`: it gates both serving `frontend/dist` in production (`app.js`) and the `secure` flag on the auth cookie (`utils/jwt.js`) — leaving it unset in a production deploy serves no frontend and ships the session cookie without `secure`.
...
- `routes/*.routes.js` mount under `/api/<name>` in `app.js` and own request validation (zod schemas defined inline per route file, applied via `validateBody` for the request body and `validateQuery` for query params — the latter lands in `req.validatedQuery` because Express 5 makes `req.query` read-only. Every query schema is `.strict()`: Express 5's `simple` query parser reads `?logDate[not]=x` as one literal key instead of a nested object, so without `.strict()` a malformed param would be silently dropped rather than rejected) plus `requireAuth` middleware. `POST /api/auth/logout` is the one route deliberately left off `requireAuth`: it must stay reachable with an expired/invalid session, and the handler doesn't touch `req.userId`.
...
- **`undici` stays on 6 on purpose — do not upgrade to 8.** `openFoodFacts.service.js` hands a `ProxyAgent` from the installed `undici` package to Node's global `fetch`, which bundles its *own* copy of undici. That coupling rests on no public contract: with undici 8 on Node 24, `fetch` rejects the dispatcher outright (`TypeError: fetch failed` ← `UND_ERR_INVALID_ARG: invalid onRequestStart method`), so behind a corporate proxy every Open Food Facts call dies. `integration/proxyDispatcher.test.js` pins this: it passes on 6 and fails on 8. Revisit only when Node's bundled undici catches up, and re-run that test to decide.
...
- Auth is a JWT in an **httpOnly cookie** (`utils/jwt.js`), not a bearer token — `requireAuth` reads it via `cookie-parser` and sets `req.userId`. `GET /api/auth/me` is how the frontend restores a session on load, since JS can't read the cookie itself. `app.js` answers unmatched `/api/*` routes with a JSON 404 (mounted after the API routers, before the SPA catch-all), so a typo'd endpoint never falls through to `index.html`. That catch-all is `app.get(/.*/, …)` with a real RegExp, not `'*'`: Express 5 rejects the bare wildcard, and a RegExp sidesteps path-to-regexp instead of depending on its syntax. It only runs under `NODE_ENV=production`, so no test covers it — the `docker` CI job curls `/` and `/history` for exactly this reason.

**Prisma 7 wiring** — the connection string lives in exactly two places, neither of them the schema. `datasource db` declares only `provider = "sqlite"`; Prisma 7 rejects a `url` there. Migrate reads it from `backend/prisma.config.js`, and the client gets it through a driver adapter in `src/lib/prisma.js` (`new PrismaBetterSqlite3({ url: env.databaseUrl })` — note the class is `PrismaBetterSqlite3`, not `...SQLite3`). Two consequences worth knowing: any Prisma CLI command must run from `backend/` so `prisma.config.js` is found (hence the `cd backend` in `docker-entrypoint.sh`), and `better-sqlite3` is a native module with no musl prebuild, so the Dockerfile's **build stage** installs `python3 make g++` to compile it. Those tools stay out of the runtime stage, which only receives the compiled `.node`. Prisma 7 also roughly doubles the image's `node_modules` (`@prisma` alone is ~180 MB).
...
- `integration/rateLimit.test.js` is deliberately its own file: express-rate-limit's MemoryStore lives as long as the app instance, and vitest reloads modules per test file, so burning the 20-request quota there cannot fail the other files. Keep an eye on this if any auth-heavy test file grows past ~20 requests to `/api/auth/*`.
...
**Frontend** (`frontend/src`) — React Router (declarative mode) + Tailwind 4, mobile-first:
- There is **no `tailwind.config.js`**: since Tailwind 4 the theme lives in CSS. `src/index.css` opens with `@import 'tailwindcss'` then an `@theme` block holding the `brand` palette (`--color-brand-50` … `--color-brand-900`), the two font stacks (`--font-sans`, `--font-display`) and `--shadow-soft`. Content paths are auto-detected, and `autoprefixer` is gone — `@tailwindcss/postcss` covers it. Add a design token by declaring a CSS variable in `@theme`, not by editing a JS config that no longer exists.
- The `@layer base` block in `index.css` that sets `border-color` on `*` is a v3 compatibility shim, not decoration: Tailwind 4 changed the default border colour to `currentcolor`, which would have recoloured every border in the app. Removing it means auditing every `border` utility for an explicit colour first.
...
- `vite.config.js` sets `resolve.dedupe: ['react', 'react-dom']`, and it is not optional. The `prisma` CLI pulls in `@prisma/studio-core`, which depends on React 18 (Radix UI, visx); npm hoists that copy to the workspace root, so without deduping Vite bundles two Reacts and every page dies on `Cannot read properties of null (reading 'useRef')`. The build succeeds either way — only running the app reveals it.
...
- `e2e/parcours.spec.js` (Playwright, `npx playwright test`) covers the browser journey end to end: register, onboarding, custom food + log, weigh-in, session restore, logout. It runs against the **Docker container**, i.e. the production bundle, not the Vite dev server — the point is to test the assets actually shipped. It needs the container up (`docker compose up -d --build --wait`); `E2E_BASE_URL` overrides the target. Each run mints unique emails from `E2E_RUN_ID` because the container's database is not reset between runs.
- The a11y tests in that file assert a **minimum field count per page** before checking labels. That is not decoration: `page.goto()` resolves before React renders, so without the count a not-yet-rendered page finds zero fields and the check passes while verifying nothing. This was a real bug in the first version of the test — it stayed green after a `htmlFor` was deliberately deleted.
- There are still no component-level unit tests for React. The backend has unit tests for its pure services plus route-level integration tests (see the **Backend tests** section above), but nothing exercises React components, and the camera path in `BarcodeScanner.jsx` can only be checked by hand on a real device.
